...
import { Check, ArrowLeft, ChevronDown, CheckCircle2, AlertCircle } from "lucide-react"
...
export default function CreateInquiryPage() {
  ...
  // Customer ID (different from user.id)
  ...
  useEffect(() => {
    Promise.all([
      api.get(`/customer/${user!.id}/user`),   // ← resolve customer ID
      api.get("/services-categories"),
      api.get("/services"),
      api.get("/device-types"),
      api.get("/device-brands"),
    ]).then(([customerRes, catRes, svcRes, typeRes, brandRes]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        customerId:     customerId,   // ← real customer ID, not user.id
        guestId:        null,
        companyId:      null,
  ...
  if (submitted) {
    return (
      <section style={{ minHeight: "100vh", background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif" }}>
        <div style={{ textAlign: "center", padding: 32 }}>
          ...
          <p style={{ fontSize: 14, color: "#64748b" }}>We'll review your request and get back to you shortly.</p>
        </div>
      </section>
    )
  }
  ...
  // ── Render ────────────────────────────────────────────────────────────────────
  return (
    <section style={{ minHeight: "100vh", background: "#f1f5f9", color: "#0f172a", fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "32px 16px" }}>
        <div style={{ borderRadius: 24, overflow: "hidden", background: "#fff", border: "1px solid #e2e8f0", boxShadow: "0 20px 50px rgba(15,23,42,0.08)" }}>
          ...
          {/* Header */}
          <div style={{ position: "sticky", top: 0, zIndex: 10, background: "rgba(255,255,255,0.92)", backdropFilter: "blur(10px)", borderBottom: "1px solid #e2e8f0" }}>
            <div style={{ padding: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
                  <div style={{ width: 50, height: 50, borderRadius: 14, background: "linear-gradient(135deg, #2563eb, #3b82f6)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 18 }}>TF</div>
                  <div>
                    <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#0f172a" }}>Techne Fixer</h1>
                    <p style={{ margin: "4px 0 0", fontSize: 13, color: "#64748b" }}>Tech Repair Services</p>
                  </div>
                </div>
                <Link to="/" style={{ fontSize: 13, color: "#475569", textDecoration: "none", display: "flex", alignItems: "center", gap: 6 }}>
                  <ArrowLeft size={14} /> Home
                </Link>
              </div>
              <div style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
                {["Fast response", "Professional", "Trusted"].map(item => (
                  <div key={item} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 999, background: "#f8fafc", border: "1px solid #e2e8f0", fontSize: 12, color: "#475569" }}>
                    <Check size={13} color="#2563eb" /> {item}
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Error Banner */}
          {error && (
            <div style={{ margin: "20px 24px 0", display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderRadius: 12, background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", fontSize: 13 }}>
          ...
          {loadingInit ? (
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ padding: 24, display: "grid", gap: 20 }}>
                ...
                {/* 1 · Service */}
                <Section number="1" title="What service do you need?" subtitle="Required">
                ...
                {/* 2 · Device */}
                <Section number="2" title="Device Details" subtitle="Optional">
                ...
                {/* 3 · Issue */}
                <Section number="3" title="Describe Your Issue" subtitle="Required">
                ...
                {/* 4 · Schedule & Location */}
                <Section number="4" title="Schedule & Location" subtitle="Optional">
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <div>
                    ...
                    <div style={{ gridColumn: "1 / -1" }}>
                      ...
                      <p style={{ fontSize: 12, color: "#94a3b8", marginTop: 8 }}>
                        You can save addresses to your account in Settings.
                      </p>
                    </div>
                  </div>
                </Section>
                ...
                <Section number="5" title="How did you find us?" subtitle="Optional">
              ...
              <div style={{ position: "sticky", bottom: 0, background: "rgba(255,255,255,0.94)", backdropFilter: "blur(10px)", borderTop: "1px solid #e2e8f0", padding: 20 }}>
                <button type="submit" disabled={submitting || !customerId}
                  style={{
                    ...
                    background: submitting || !customerId ? "#94a3b8" : "#2563eb",
                    color: "#fff", fontSize: 14, fontWeight: 700, padding: "14px 18px",
                    cursor: submitting || !customerId ? "not-allowed" : "pointer",
