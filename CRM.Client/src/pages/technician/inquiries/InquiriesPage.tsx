...
type Status = "Assigned" | "In Diagnosis" | "Quoted" | "Closed"
...
const MY_INQUIRIES: TechInquiry[] = [
  {
    id: "INQ-1040",
  ...
  },
  {
    id: "INQ-1035",
    customer: "Luis Gomez",
    address: "11 Acacia Ave., General Santos",
    service: "Pipe Replacement",
    urgency: "Medium",
    preferredDate: "May 9, 2026",
    assignedAt: "May 4, 9:30 AM",
    status: "In Diagnosis",
    hasNote: false,
  },
  {
    id: "INQ-1038",
    customer: "Ethan Park",
    address: "88 Rizal Blvd., Koronadal",
    service: "Boiler Maintenance",
    urgency: "Low",
    preferredDate: "May 12, 2026",
    assignedAt: "May 5, 2:15 PM",
    status: "Assigned",
    hasNote: false,
  },
  {
    ...
    address: "5 Sampaguita Rd., Davao City",
    service: "Electrical Wiring",
    urgency: "High",
    preferredDate: "May 7, 2026",
    assignedAt: "May 2, 11:00 AM",
    status: "Assigned",
    hasNote: false,
  },
  {
    id: "INQ-1029",
    customer: "Maria Villanueva",
    address: "22 Mango St., Cotabato City",
    service: "HVAC Repair",
    urgency: "Medium",
    preferredDate: "May 6, 2026",
    assignedAt: "Apr 30, 8:45 AM",
    status: "Quoted",
    hasNote: true,
  },
  {
    id: "INQ-1027",
    customer: "Rey Dela Cruz",
    address: "3 Narra Compound, Tacurong",
    service: "AC Installation",
    urgency: "Low",
    preferredDate: "May 3, 2026",
    assignedAt: "Apr 28, 3:20 PM",
    status: "Closed",
    hasNote: true,
  },
]
...
const URGENCY_STYLE: Record<Urgency, { color: string; bg: string; icon: string }> = {
  "High":   { color: "#f87171", bg: "rgba(248,113,113,0.1)",  icon: "🔴" },
  "Medium": { color: "#fbbf24", bg: "rgba(245,158,11,0.1)",   icon: "🟡" },
  "Low":    { color: "#34d399", bg: "rgba(16,185,129,0.1)",   icon: "🟢" },
}
...
export default function TechnicianInquiriesPage() {
  ...
  const techName  = "Paulo Mendez"
  const initials  = "PM"

  const filtered = MY_INQUIRIES
    .filter(inq => {
    ...
    .sort((a, b) => {
      // Active work first, then by urgency
  ...
  const activeCount = MY_INQUIRIES.filter(i =>
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 13, color: "#64748b", margin: "3px 0 0" }}>
              {techName} · {activeCount} active assignment{activeCount !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {[
            { label: "In Diagnosis", count: MY_INQUIRIES.filter(i => i.status === "In Diagnosis").length, color: "#60a5fa" },
            { label: "Assigned",     count: MY_INQUIRIES.filter(i => i.status === "Assigned").length,     color: "#fbbf24" },
            { label: "Quoted",       count: MY_INQUIRIES.filter(i => i.status === "Quoted").length,       color: "#34d399" },
          ].map(stat => (
            ...
            >
              ...
              {tab.value !== "All" && (
                <span style={{
                  ...
                  {MY_INQUIRIES.filter(i => i.status === tab.value).length}
                </span>
              )}
            </button>
          ))}
          ...
          <div>
            {filtered.map((inq, idx) => (
              <InquiryRow
                ...
                onView={() => navigate(`/technician/inquiries/${inq.id}`)}
                onDiagnose={() => navigate(`/technician/diagnosis/${inq.id}`)}
              />
            ))}
          </div>
        )}
...
function InquiryRow({
  ...
  const isActive     = inq.status === "Assigned" || inq.status === "In Diagnosis"
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        background: isClosed ? "transparent" : urgencyStyle.color,
        opacity: 0.7,
      }} />
      ...
      <div style={{
        ...
        <div style={{ minWidth: 180, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{
              ...
              {inq.customer.split(" ").map((n: string) => n[0]).join("")}
            </div>
        ...
        <div style={{ minWidth: 120, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 12, color: "#64748b" }}>Preferred</span>
          </div>
        ...
        <div style={{ minWidth: 80, flexShrink: 0 }}>
          <span style={{
            ...
            <span style={{
              ...
              background: urgencyStyle.color, flexShrink: 0,
